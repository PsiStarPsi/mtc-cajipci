...
int altix_event_size()
{
	return BEAM_TEST_EVENT_SIZE;
}
int altix_packet_size()
{
...
int altix_lock_card(int fd, int id)
{
	...
	if(ioctl(fd, ALTIX_IOCTL_LOCK, id) == 0)
	{
#ifdef ALTIX_DEBUG
...
int altix_send_pulse(int fd, int fpga)
{
	...
	if(!is_initialized()) return 0;
	if(fpga)
		cmd = ALTIX_IOCTL_FANCY_CMD_MON_1;
	else
		cmd = ALTIX_IOCTL_FANCY_CMD_MON_0;
	if(ioctl(fd, ALTIX_IOCTL_FANCY, &cmd) == 0)
		return ALTIX_OK;
	if(errno == EBUSY)
		return ALTIX_CARD_BUSY;
	return ALTIX_CARD_NOT_VALID;	
...
{
	if(!is_initialized()) return 0;
	*ret = ALTIX_IOCTL_FANCY_CMD_PULSE_READ;
	if(ioctl(fd, ALTIX_IOCTL_FANCY, ret) == 0)
		return ALTIX_OK;
	if(errno == EBUSY)
		return ALTIX_CARD_BUSY;
	return ALTIX_CARD_NOT_VALID;	
